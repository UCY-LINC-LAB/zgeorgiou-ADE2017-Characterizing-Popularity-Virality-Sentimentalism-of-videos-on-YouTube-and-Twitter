...
public class StatusMonitor implements Runnable {
    ...
    public void run() {
        ...
        while(true){
            ...
            //Start of New Code
            boolean isOriginal = !status.isRetweet();

            //The status is original
            if(isOriginal){

            }else{ // The status is retweet

                //Check if the original tweet, has a video.

                //If the video is already monitored, we should add this tweet as retweet

                //If is not monitored and we have extra places for new videos

                // We add both the original and the retweet video
