...
public class DBVideosService implements DBVideosI {
    ...
    public DBVideosService(DBConnection dbConnection, MongoCollection videos, Arguments arguments) {
    ...
    public int getTotalTotallyFinishedVideos() {
        return (int) videos.count( and(
                eq("meta.finished",true),
                eq("meta.comments_finished",true),
                eq("meta.incomplete",false)));
    }
    @Override
    ...
    @Override
    public boolean checkVideoIfExistsInDB(String video_id) {
    ...
    public boolean checkVideoIsFinished(String video_id) {
        if(arguments.isDebug_mode())
            return videos.count(and(eq("_id",video_id),eq("meta.current_date",1)))==1;
        return videos.count(and(eq("_id",video_id),eq("meta.current_date",16)))==1;
    }

    public long getVideoTimestamp(String video_id){
       Document doc =(Document) videos.find(eq("_id",video_id))
                .projection(fields(include("meta.timestamp"))).first();
       if(doc!=null){
           return ((Document)doc.get("meta")).getLong("timestamp");
       }
       return 0;
    }
    public boolean videoExists(String video_id){
        return videos.count(eq("_id",video_id)) == 1;
    }
    @Override
