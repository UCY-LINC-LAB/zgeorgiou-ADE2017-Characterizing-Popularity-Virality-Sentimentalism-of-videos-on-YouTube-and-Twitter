...
public  class DBServices implements DBServicesI{
    ...
    public DBServices(DBConnection connection, Arguments arguments) {
        ...
        Document keyI = new Document("_id",1);
        Document keyU = new Document("user_id",1);
        tweets.createIndex(keys);
        tweets.createIndex(keyI);
        tweets.createIndex(keyU);

        comments.createIndex(keys);

        dbVideosService  = new DBVideosService(connection,videos, arguments);
    ...
    public boolean addTweet(long tweetId, JsonObject tweet) {
        ...
        document.append("_id",tweetId)
                ...
                .append("user_id",tweet.get("user_id").getAsLong())
    ...
    @Override
    public boolean containsTweet(long tweet_id) {
        Document document = (Document) tweets.find(eq("_id",tweet_id)).first();
        if(document==null){
            return false;
        }
        return true;
    ...
    public Document createDefaultConfiguration(){
        ...
        int default_max_videos_being_monitored = arguments.getMax_videos();
